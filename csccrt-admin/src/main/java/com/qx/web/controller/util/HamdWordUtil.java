...
public class HamdWordUtil {
    ...
    public static String getDld(EisUser eisUser, EisRecord eisRecord, String templateurl) {
        ...
        String fileName=eisUser.getUserName()+"_"+eisRecord.getTableId()+filenameRandom+"_hamd.docx";  // 结果文件
    ...
    public static void doParagraphs(XWPFDocument doc,EisUser eisUser,EisRecord eisRecord) {
        ...
        Map<String, String> textMap = new HashMap<String, String>();
         String string="";
         if(eisRecord.getScore()>35){
             string=string+"可能为严重抑郁。";
         }else if(20<eisRecord.getScore() && eisRecord.getScore()<=35){
             string=string+"可能有轻度或中度抑郁。";
         }else if(8<=eisRecord.getScore() && eisRecord.getScore()<=20){
             string=string+"可能有抑郁。";
         }else{
             string=string+"没有抑郁症状。";
         }
            textMap.put("var", "总分低于8分者没有抑郁症状；" +
                    "总分为8~20分者可能有抑郁；" +
                    "总分为21~35分者有轻度或中度抑郁；" +
                    "总分高于35分者可能为严重抑郁。本次测量总分"+eisRecord.getScore()+"分，"+string);
        SimpleDateFormat sdf =new SimpleDateFormat("yyyy年MM月dd日");
        Date date =new Date();
        String f=sdf.format(date);
            textMap.put("varA",String.valueOf(eisRecord.getScoreA()));
            textMap.put("varB",String.valueOf(eisRecord.getScoreB()));
            textMap.put("varC",String.valueOf(eisRecord.getScoreC()));
            textMap.put("varD",String.valueOf(eisRecord.getScoreD()));
            textMap.put("varE",String.valueOf(eisRecord.getScoreE()));
            textMap.put("varF",String.valueOf(eisRecord.getScoreF()));
            textMap.put("varG",String.valueOf(eisRecord.getScoreG()));
            textMap.put("varH",String.valueOf(eisRecord.getScore()));
            textMap.put("var1",f);
        // 图片数据
