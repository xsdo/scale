...
public class GdsWord {
    ...
    public static String getDld(ScalePatient scalePatient , ScaleTask scaleTask,String templateurl,List<ScaleScore> scaleScoreList) {
        ...
        String fileName=scalePatient.getPatientName()+"_"+scaleTask.getTestCoding()+filenameRandom+"_gds.docx";  // 结果文件
...
//        doCharts(doc,scaleScoreList);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    ...
    public static void doParagraphs(XWPFDocument doc,ScalePatient scalePatient ,ScaleTask scaleTask,List<ScaleScore> scaleScoreList) {
        ...
        for (int i=0;i<scaleScoreList.size();i++){
            ScaleScore scaleScore=scaleScoreList.get(i);
            score+=scaleScore.getScore();
            textMap.put("var"+scaleScore.getTitle(),scaleScore.getScore()+"");
        }
        if (score<=10){
            string="您目前无抑郁症状。请继续保持好心情！";
        }else if (score>=11&&score<=20){
            string="您目前存在轻度抑郁症状。可尝试想一些开心的事儿，转移注意力。";
        }else if (score>=21&&score<=30){
            string="您目前存在中重度抑郁症状。建议及时寻求专业的心理治疗，必要时配合药物治疗。";
        }
