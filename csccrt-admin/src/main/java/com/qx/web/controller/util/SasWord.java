...
public class SasWord {
    ...
    public static String getDld(ScalePatient scalePatient , ScaleTask scaleTask,String templateurl,List<ScaleScore> scaleScoreList) {
        ...
        String fileName=scalePatient.getPatientName()+"_"+scaleTask.getTestCoding()+filenameRandom+"_sas.docx";  // 结果文件
...
//        doCharts(doc);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    ...
    public static void doParagraphs(XWPFDocument doc,ScalePatient scalePatient ,ScaleTask scaleTask,List<ScaleScore> scaleScoreList) {
        ...
        Long score=50L;
        if (score<50){
            str="表明您目前不存在焦虑，请继续保持好心情！";
        }else if (score>=50&&score<60){
            str="表明您目前存在轻度焦虑。建议进行放松训练或做一些自己感兴趣的事儿，必要时可进行心理治疗。";
        }else if (score>=60&&score<70){
            str="表明您目前存在中度焦虑。建议进行心理治疗，必要时可使用抗焦虑药物。";
        }else {
            str="表明您目前存在重度焦虑。建议联合使用药物治疗与心理治疗。";
        }
        textMap.put("varA",scoreC+string);
        textMap.put("varB",score+string);
        textMap.put("var", "本次测验标准分为"+score+"分，"+str);
