...
public class SclWord {
    ...
    public static String getDld(ScalePatient scalePatient , ScaleTask scaleTask,String templateurl,List<ScaleScore> scaleScoreList) {
        ...
        String fileName=scalePatient.getPatientName()+"_"+scaleTask.getTestCoding()+filenameRandom+"_scl90.docx";  // 结果文件
    ...
    public static void doParagraphs(XWPFDocument doc,ScalePatient scalePatient ,ScaleTask scaleTask,List<ScaleScore> scaleScoreList) {
        ...
        int count=0;
        Long score=scoreA+scoreB+scoreC+scoreD+scoreE+scoreF+scoreG+scoreH+scoreI+scoreJ;
        // 文本数据
        ...
        if (s1<=2&&s2<=2&&s3<=2&&s4<=2&&s5<=2&&s6<=2&&s7<=2&&s8<=2&&s9<=2&&s10<=2){
            string="阳性项目数为0项。表明近期您的身心处于健康水平，请继续保持！";
        }else {
            if (s1>2){
                strings.add("躯体化");
