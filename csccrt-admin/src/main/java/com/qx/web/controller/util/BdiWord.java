...
public class BdiWord {
    ...
    public static String getDld(ScalePatient scalePatient , ScaleTask scaleTask,String templateurl,List<ScaleScore> scaleScoreList) {
        ...
        String fileName=scalePatient.getPatientName()+"_"+scaleTask.getTestCoding()+filenameRandom+"_bdi.docx";  // 结果文件
...
//        doCharts(doc,scaleScoreList);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    ...
    public static void doParagraphs(XWPFDocument doc,ScalePatient scalePatient ,ScaleTask scaleTask,List<ScaleScore> scaleScoreList) {
        ...
        for (ScaleScore scaleScore:scaleScoreList){
            ...
            }else if (scaleScore.getTitle().equals("10")){
                scoreJ=scaleScore.getScore();
            }else if (scaleScore.getTitle().equals("11")){
                scoreK=scaleScore.getScore();
            }else if (scaleScore.getTitle().equals("12")){
                scoreL=scaleScore.getScore();
            }else if (scaleScore.getTitle().equals("13")){
                scoreM=scaleScore.getScore();
            }
        }
        ...
        textMap.put("varK",scoreK+"");
        textMap.put("varL",scoreL+"");
        textMap.put("varM",scoreM+"");
        textMap.put("varN",scoreN+"");
        if (scoreN<=4){
            string="您目前不存在抑郁症状。请继续保持好心情。";
        }else if (scoreN>=5&&scoreN<=7){
            string="您目前存在轻度抑郁。建议进行放松训练或寻求心理治疗。";
        }else if (scoreN>=8&&scoreN<=15){
            string="您目前存在中度抑郁。建议及时进行心理治疗，必要时配合药物治疗。";
        }else if (scoreN>=16){
            string="您目前存在重度抑郁。建议联合使用药物治疗与心理治疗。";
        }
        textMap.put("var", "本次测验总分为"+scoreN+"分，表明"+string);
