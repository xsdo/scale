...
public class MdqWord {
    ...
    public static String getDld(ScalePatient scalePatient , ScaleTask scaleTask,String templateurl,List<ScaleScore> scaleScoreList,List<ScaleQuestions>scaleQuestionsList) {
        ...
        String fileName=scalePatient.getPatientName()+"_"+scaleTask.getTestCoding()+filenameRandom+"_mdq.docx";  // 结果文件
...
//        doCharts(doc,scaleScoreList);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    ...
    public static void doParagraphs(XWPFDocument doc,ScalePatient scalePatient ,ScaleTask scaleTask,List<ScaleScore> scaleScoreList,List<ScaleQuestions>scaleQuestionsList) {
        ...
        for (ScaleScore scaleScore:scaleScoreList){
            //排除14.15题其他部分项
            if (scaleScore.getTitle().equals("14")){
                score14=scaleScore.getScore();
            }else if (scaleScore.getTitle().equals("15")){
                score15=scaleScore.getScore();
            }else{
                ...
                for (ScaleQuestions scaleQuestions:scaleQuestionsList){
                    if (scaleScore.getTitle().equals(scaleQuestions.getTitle())&&scaleScore.getScore()>0){
                        questionsList.add(scaleQuestions.getContent());
                    }
                }
            }
        ...
        if (score>=7&&score14>0&&score15>1){
            str="筛查症状同时存在，";
            if (score15==2){
                str2="中度";
            }else {
            ...
            string=str+"症状的严重程度为"+str2+"，表明双相障碍筛查阴性。";
        }else {
            if (score14>0){
                str="筛查症状同时存在，";
            }else {
            ...
            if (score15==0){ str2="没有问题"; }
            if (score15==1){ str2="轻度"; }
            if (score15==2){ str2="中度"; }
            if (score15==3){ str2="严重问题"; }
            string=str+"症状的严重程度为"+str2+"，表明双相障碍筛查阴性。";
        }
                        ...
                        if (text.contains("${table2}")) {
                            ...
                            if (textMap.get("score").equals("0")){
                                ...
                                for (int j=0;j<questionsList.size();j++){
                                    ...
                                    new PoiWordTools().setWordCellSelfStyle(tableOneRowTwo.getCell(1), "宋体", "10", 0, "center", "top", "#000000", "#ffffff", "80%", questionsList.get(j));
