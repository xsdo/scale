...
public class EisWord {

    public static String getDld(EisDomain eisDomain, String templateurl) {
        FileSystemView fsv = FileSystemView.getFileSystemView();
        File com=fsv.getHomeDirectory();
        PatientUser patientUser=eisDomain.getPatientUser();
        String fileName=eisDomain.getPatientUser().getPatientName()+"_"+eisDomain.getPatientUser().getTestCoding()+"_eis.docx";  // 结果文件
        ...
        try {
            ...
            replaceAll(doc,patientUser,eisDomain);
     ...
     */
    public static void replaceAll(XWPFDocument doc,PatientUser patientUser,EisDomain eisDomain) throws InvalidFormatException, IOException {
        doParagraphs(doc,patientUser,eisDomain); // 处理段落文字数据，包括文字和表格、图片
        doCharts(doc,eisDomain);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    }
     ...
     * @throws IOException
     */
    public static void doParagraphs(XWPFDocument doc,PatientUser patientUser,EisDomain eisDomain) {
        ...
        Map<String, String> textMap = new HashMap<String, String>();
         String string="";
         if(eisDomain.getSum()>120){
             string=string+"情绪智力水平很高。";
         }else if(82.5<eisDomain.getSum() && eisDomain.getSum()<=120){
             string=string+"情绪智力水平较高。";
         }else if(45<=eisDomain.getSum() && eisDomain.getSum()<=82.5){
            ...
            textMap.put("var", "情绪智力量表旨在测量个体感知、理解、表达、控制和利用自己以及他人情绪的能力。得分越高表明情绪智力水平越高。量表的高分者通常更为积极、更能克制冲动、更清楚地表达自己的感受、" +
                    "更好地恢复、较少的情感障碍和抑郁、更富同情心、更能自我监控。该量表总分165分，本次测量总分"+eisDomain.getSum()+"分，"+string);
            textMap.put("var1",eisDomain.getCreateTime());
        // 图片数据
    ...
    public static void doCharts(XWPFDocument doc,EisDomain eisDomain) {
        /**----------------------------处理图表------------------------------------**/
