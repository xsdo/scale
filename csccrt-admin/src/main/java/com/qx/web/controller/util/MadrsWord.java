...
public class MadrsWord {
    ...
    public static String getDld(ScalePatient scalePatient , ScaleTask scaleTask,String templateurl,List<ScaleScore> scaleScoreList) {
        ...
        String fileName=scalePatient.getPatientName()+"_"+scaleTask.getTestCoding()+filenameRandom+"_madrs.docx";  // 结果文件
...
//        doCharts(doc,scaleScoreList);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    ...
    public static void doParagraphs(XWPFDocument doc,ScalePatient scalePatient ,ScaleTask scaleTask,List<ScaleScore> scaleScoreList) {
        ...
        if (scoreK<=11){
            string="您目前没有抑郁症状。";
        }else if (scoreK>=12&&scoreK<=23){
            string="您目前可能存在轻度的抑郁症状。可尝试做一些令你感到愉快的事儿，使身心放松。";
        }else if (scoreK>=24&&scoreK<=34){
            string="您目前可能存在中度抑郁。建议及时寻求心理治疗，必要时可结合药物治疗。";
        }else {
            string="您目前可能存在重度抑郁。建议联合使用药物治疗与心理治疗。";
        }
        textMap.put("var", "本次测验总分为"+scoreK+"分，表明"+string);
