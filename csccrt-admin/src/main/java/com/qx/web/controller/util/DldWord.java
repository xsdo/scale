...
public class DldWord {

    public static String getDld(DldDomain dldDomain, String templateurl) {
        FileSystemView fsv = FileSystemView.getFileSystemView();
        File com=fsv.getHomeDirectory();
        String fileName=dldDomain.getPatientUser().getPatientName()+"_"+dldDomain.getPatientUser().getTestCoding()+"_dld.docx";
     ...
     */
    public static void replaceAll(XWPFDocument doc,DldDomain dldDomain) throws InvalidFormatException, IOException {
        doParagraphs(doc,dldDomain); // 处理段落文字数据，包括文字和表格、图片
        doCharts(doc,dldDomain);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    }
     ...
     * @throws IOException
     */
    public static void doParagraphs(XWPFDocument doc,DldDomain dldDomain) {
        PatientUser patientUser=dldDomain.getPatientUser();
        ...
        if(dldDomain.getSum()>=61){
            textMap.put("var", "该量表总分"+dldDomain.getSum()+"分，表明该患者存在述情障碍。不能很好的描述、辨别自己的情感，缺乏透露内在的态度、感受、愿望和欲念的能力，执着于外界事物的细枝末节。");
        }else if(dldDomain.getSum()<=51){
            textMap.put("var", "该量表总分"+dldDomain.getSum()+"分，表明该患者不存在述情障碍。能够较好的描述、辨别自己的情感，善于表达内在的态度、感受、愿望和欲念，不执着于外界事物的细枝末节。");
        }else {
            textMap.put("var", "该量表总分"+dldDomain.getSum()+"分，存在述情障碍倾向。在描述、辨别自己的情感，或表达内在的态度、感受、愿望和欲念等方面存在一定的困难。");
        }
        textMap.put("var1", dldDomain.getCreateTime());
        // 图片数据
                ...
                for (XWPFRun run : runs) {
                    ...
                    if (text != null) {
                        ...
                        if (text.contains("${table1}")) {
                            ...
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowOne.getCell(0), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "姓名："+patientUser.getPatientName());
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowOne.createCell(), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "性别："+patientUser.getSex());
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowOne.createCell(), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "婚姻："+patientUser.getMaritalStatus());

                            // 表格第二行
                            XWPFTableRow tableOneRowTwo = tableOne.createRow();//行
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowTwo.getCell(0), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "出生年月："+patientUser.getBirth());
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowTwo.getCell(1), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "文化程度："+patientUser.getEducation());
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowTwo.getCell(2), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "职业："+patientUser.getJob());

                            // 表格第三行
                            XWPFTableRow tableOneRowThree = tableOne.createRow();//行
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowThree.getCell(0), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "住院号："+patientUser.getHospitalNumber());
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowThree.getCell(1), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "病区："+patientUser.getWard());
                            new PoiWordTools().setWordCellSelfStyle(tableOneRowThree.getCell(2), "宋体", "14", 0, "left", "top", "#000000", "#ffffff", "30%", "心测号："+patientUser.getTestCoding());
    ...
    public static void doCharts(XWPFDocument doc,DldDomain dldDomain) {
        ...
        base4.put("item1", "总分");
        base4.put("item2", String.valueOf(dldDomain.getSum()));
