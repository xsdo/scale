...
public class ShtlWord {

    public static String getDld(ShtlDomain shtlDomain, String templateurl) {
        FileSystemView fsv = FileSystemView.getFileSystemView();
        File com=fsv.getHomeDirectory();
        PatientUser patientUser=shtlDomain.getPatientUser();
        String fileName=patientUser.getPatientName()+"_"+patientUser.getTestCoding()+"_shtl.docx";  // 结果文件
        ...
        try {
            ...
            replaceAll(doc,patientUser,shtlDomain);
     ...
     */
    public static void replaceAll(XWPFDocument doc,PatientUser patientUser,ShtlDomain shtlDomain) throws InvalidFormatException, IOException {
        doParagraphs(doc,patientUser,shtlDomain); // 处理段落文字数据，包括文字和表格、图片
        doCharts(doc,shtlDomain);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    }
     ...
     * @throws IOException
     */
    public static void doParagraphs(XWPFDocument doc,PatientUser patientUser,ShtlDomain shtlDomain) {
        ...
        if(shtlDomain.getCorrect()==0){
            textMap.put("var", "社会推理任务旨在测查个体根据故事情境进行逻辑推理的能力。本次测试的正确率为"+shtlDomain.getCorrect()+"%，表明社会推理能力很差。");
        }else if(shtlDomain.getCorrect()==25){
            textMap.put("var", "社会推理任务旨在测查个体根据故事情境进行逻辑推理的能力。本次测试的正确率为"+shtlDomain.getCorrect()+"%，表明社会推理能力较差。");
        }else if(shtlDomain.getCorrect()==50) {
            textMap.put("var", "社会推理任务旨在测查个体根据故事情境进行逻辑推理的能力。本次测试的正确率为"+shtlDomain.getCorrect()+"%，表明社会推理能力一般。");
        }else if(shtlDomain.getCorrect()==75) {
            textMap.put("var", "社会推理任务旨在测查个体根据故事情境进行逻辑推理的能力。本次测试的正确率为"+shtlDomain.getCorrect()+"%，表明社会推理能力较好。");
        }else{
            textMap.put("var", "社会推理任务旨在测查个体根据故事情境进行逻辑推理的能力。本次测试的正确率为"+shtlDomain.getCorrect()+"%，表明社会推理能力很好。");
        }
        textMap.put("var1", shtlDomain.getCreateTime());
        // 图片数据
    ...
    public static void doCharts(XWPFDocument doc,ShtlDomain shtlDomain) {
        /**----------------------------处理图表------------------------------------**/
        ...
        base1.put("item1", "正确率");
        base1.put("item2", String.valueOf(shtlDomain.getCorrect()));

        // 第二行数据
        Map<String, String> base2 = new HashMap<String, String>();
        base2.put("item1", "错误率");
        base2.put("item2", String.valueOf(100-shtlDomain.getCorrect()));
