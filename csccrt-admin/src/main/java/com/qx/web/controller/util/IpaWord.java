...
public class IpaWord {
    ...
    public static String getIpa(IpaDomain ipaDomain, String templateurl) {
        ...
        String fileName=ipaDomain.getIpaPatientUser().getPatientName()+"_"+ipaDomain.getIpaPatientUser().getTestCoding()+"_ipa.docx";
    ...
    public static void doParagraphs(XWPFDocument doc,IpaDomain ipaDomain) {
        ...
        double covid = ipaDomain.getCovid();
        if(covid<5){
            textMap.put("varA", "分数为"+covid+"分，.....。");
        }else if(covid>=5 && covid<=6){
            textMap.put("varA", "分数为"+covid+"分，......。");
        }else if(covid>6 && covid<=7){
            textMap.put("varA", "分数为"+covid+"分，.......。");
        }else{
            textMap.put("varA", "分数为"+covid+"分，........。");
        }
        ...
        textMap.put("var2", Integer.toString(ipaDomain.getCovid()));
        // 图片数据
    ...
    /*public static void doCharts(XWPFDocument doc, IpaDomain ipaDomain) {
        *//**----------------------------处理图表------------------------------------**//*
        ...
        base1.put("item1", "焦虑量表");
        base1.put("item2", String.valueOf(ipaDomain.getSas()));

        // 第二行数据
        Map<String, String> base2 = new HashMap<String, String>();
        base2.put("item1", "抑郁量表");
        base2.put("item2", String.valueOf(ipaDomain.getSds()));

        // 第三行数据
        Map<String, String> base3 = new HashMap<String, String>();
        base3.put("item1", "阿森斯量表");
        base3.put("item2", String.valueOf(ipaDomain.getAis()));

        // 第四行数据
        Map<String, String> base4 = new HashMap<String, String>();
        base4.put("item1", "总分");
        base4.put("item2", String.valueOf(ipaDomain.getSum()));
