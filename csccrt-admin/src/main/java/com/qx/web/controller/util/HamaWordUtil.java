...
public class HamaWordUtil {
    ...
    public static String getDld(EisUser eisUser, EisRecord eisRecord, String templateurl) {
        ...
        String fileName=eisUser.getUserName()+"_"+eisRecord.getTableId()+filenameRandom+"_hama.docx";  // 结果文件
    ...
    public static void doParagraphs(XWPFDocument doc,EisUser eisUser,EisRecord eisRecord) {
        ...
        Map<String, String> textMap = new HashMap<String, String>();
         String string="";
         if(eisRecord.getScore()>29){
             string=string+"可能为严重焦虑。";
         }else if(21<eisRecord.getScore() && eisRecord.getScore()<=29){
             string=string+"有明显焦虑症状。";
         }else if(14<eisRecord.getScore() && eisRecord.getScore()<=21){
             string=string+"有焦虑症状。";
         }else if(7<=eisRecord.getScore() && eisRecord.getScore()<=14){
             string=string+"可能有焦虑症状。";
         }else{
             string=string+"没有焦虑症状。";
         }
            textMap.put("var", "总分低于7分者没有焦虑症状；" +
                    "总分为7~14分者可能有焦虑；" +
                    "总分为15~21分者有焦虑；" +
                    "总分为22~29分者有明显焦虑；" +
                    "总分高于29分者可能为严重焦虑。本次测量总分"+eisRecord.getScore()+"分，"+string);
        textMap.put("varA",String.valueOf(eisRecord.getScoreA()));
        textMap.put("varB",String.valueOf(eisRecord.getScoreB()));
        textMap.put("varC",String.valueOf(eisRecord.getScore()));
