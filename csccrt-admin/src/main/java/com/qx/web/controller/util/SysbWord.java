...
public class SysbWord {

    public static String getDld(SysbDomain sysbDomain, String templateurl) {
        FileSystemView fsv = FileSystemView.getFileSystemView();
        File com=fsv.getHomeDirectory();
        PatientUser patientUser=sysbDomain.getPatientUser();
        String fileName=patientUser.getPatientName()+"_"+patientUser.getTestCoding()+"_sysb.docx";  // 结果文件
        ...
        try {
            ...
            replaceAll(doc,patientUser,sysbDomain);
     ...
     */
    public static void replaceAll(XWPFDocument doc,PatientUser patientUser,SysbDomain sysbDomain) throws InvalidFormatException, IOException {
        doParagraphs(doc,patientUser,sysbDomain); // 处理段落文字数据，包括文字和表格、图片
        doCharts(doc,sysbDomain);  // 处理图表数据，柱状图、折线图、饼图啊之类的
    }
     ...
     * @throws IOException
     */
    public static void doParagraphs(XWPFDocument doc,PatientUser patientUser,SysbDomain sysbDomain) {
        ...
        Map<String, String> textMap = new HashMap<String, String>();
         String string="";
         if(sysbDomain.getSywt()>43.8){
             string="很高";
         }else if(30<sysbDomain.getSywt() && sysbDomain.getSywt()<=43.8){
             string="较高";
         }else if(16.2<=sysbDomain.getSywt() && sysbDomain.getSywt()<=30){
            string="较低";
         }else{
             string="很低";
         }
        String string1="";
        if(sysbDomain.getKzwt()>14.6){
            string1="很高";
        }else if(10<sysbDomain.getKzwt() && sysbDomain.getKzwt()<=14.6){
            string1="较高";
        }else if(5.4<=sysbDomain.getKzwt() && sysbDomain.getKzwt()<=10){
            string1="较低";
        }else{
            string1="很低";
        }
         textMap.put("var", "失言识别任务通过失言问题考察个体对失言者失言情境的认知，" +
                 "通过控制问题考察个体的语言理解能力。失言问题的总分为60分，" +
                 "控制问题的总分为20分。在本次测试中，失言问题得分为"+sysbDomain.getSywt()+"分，控制问题得分为"+sysbDomain.getKzwt()+"分，" +
                 "表明您识别失言行为的能力"+string+"，言语理解能力"+string1+"。");
        textMap.put("var1", sysbDomain.getCreateTime());
        // 图片数据
    ...
    public static void doCharts(XWPFDocument doc,SysbDomain sysbDomain) {
        /**----------------------------处理图表------------------------------------**/
        ...
        base1.put("item1", "失言问题");
        base1.put("item2", String.valueOf(sysbDomain.getSywt()));

        // 第二行数据
        Map<String, String> base2 = new HashMap<String, String>();
        base2.put("item1", "控制问题");
        base2.put("item2", String.valueOf(sysbDomain.getKzwt()));

        // 第三行数据
        Map<String, String> base3 = new HashMap<String, String>();
        base3.put("item1", "总分");
        base3.put("item2", String.valueOf(sysbDomain.getSum()));
