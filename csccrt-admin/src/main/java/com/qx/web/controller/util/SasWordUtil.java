...
public class SasWordUtil {
    ...
    public static String getDld(EisUser eisUser, EisRecord eisRecord, String templateurl) {
        ...
        String fileName=eisUser.getUserName()+"_"+eisRecord.getTableId()+filenameRandom+"_sas.docx";  // 结果文件
    ...
    public static void doParagraphs(XWPFDocument doc,EisUser eisUser,EisRecord eisRecord) {
        ...
        Map<String, String> textMap = new HashMap<String, String>();
         String string="";
        textMap.put("varA",eisRecord.getScoreA()+"分");
        int endScore=eisRecord.getScoreB();
        textMap.put("varB",endScore+"分");
         if(endScore>70){
             string=string+"您的焦虑情况为重度焦虑。";
         }else if(60<endScore && endScore<=70){
             string=string+"您的焦虑情况为中度焦虑。";
         }else if(50<=endScore && endScore<=60){
             string=string+"您的焦虑情况为轻度焦虑。";
         }else{
             string=string+"您的焦虑情况为正常。";
         }
            textMap.put("var", "标准分低于50分者为正常；\n" +
                    "标准分为50~60分者为轻度焦虑；\n" +
                    "标准分为61~70分者为中度焦虑；\n" +
                    "标准分高于70分者为重度焦虑；\n"  +
                    "本次测量总分"+endScore+"分，"+string);
